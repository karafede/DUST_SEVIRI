...
[Dust_daily_sum{d1:d2}] = deal(zeros(1500));
[Dust_daily_each_time_step{d1:d2,1:96}] = deal(zeros(1500));
        ...
        try
            for jj = 1:24
                ...
                if B1.(fields1{1})(:,:,jj) == 0 | B2.(fields2{1})(:,:,jj) == 0 | B3.(fields3{1})(:,:,jj) == 0
                    ...
                    if B1.(fields1{1})(:,:,jj) == 0
                        count5 = count5 + 1;
                        Zeros_layer{count5} = strcat('T10_',datestr(n,'yyyymmdd'),'_P_',num2str(m),'layer',num2str(jj),'.mat');
                    end
                    if B2.(fields2{1})(:,:,jj) == 0
                        count5 = count5 + 1;
                        Zeros_layer{count5} = strcat('T09_',datestr(n,'yyyymmdd'),'_P_',num2str(m),'layer',num2str(jj),'.mat');
                    end
                    if B3.(fields3{1})(:,:,jj) == 0
                        count5 = count5 + 1;
                        Zeros_layer{count5} = strcat('T07_',datestr(n,'yyyymmdd'),'_P_',num2str(m),'layer',num2str(jj),'.mat');
                    end
                    continue
                else
...
save ('Dust_daily_SDF_2012_1to3_end','Dust_daily_each_time_step','-v7.3')
save ('Dust_SDF_frequency_2012_1to3_end','Dust_daily_sum','-v7.3')
save ('Dust_SDF_frequency_raster_2012_1to3_end','Dust_monthly')
save ('Missing_layer_counter_2012_1to3_end','Missing_layer_counter')
save ('Missing_file_2012_1to3_end','Missing_file')
save ('Missing_index_2012_1to3_end','Missing_index')
save ('Zeros_layer_2012_1to3_end','Zeros_layer')