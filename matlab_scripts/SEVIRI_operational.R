...
patt <- ".nc"
filenames <- list.files(pattern = patt)  # it should include the current daytime
...
# create a directory for the "today date"
dir.create(paste0(year,"_",month,"_", day,"_","tif_images"))
current_dir <- getwd()
...
#    writeRaster(r, paste0(current_dir, "/tif_images/",DateTime,".tif") , options= "INTERLEAVE=BAND", overwrite=T)
    writeRaster(r, paste0(current_dir,"/", year,"_",month,"_", day,"_","tif_images/",DateTime,".tif") , options= "INTERLEAVE=BAND", overwrite=T)
