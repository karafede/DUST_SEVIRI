...
for n = n1:n2
    ...
    for t = n - 2:n - 1 %% the last 15 days for reference
       count1 = 0;
       for k =1:4    % loop for the quarter of the day (4 files per day)
           % A1 is T07 and A2 is T09
            ...
            try
                if t == n - 2 % check if the first day of ref. no need to maximize
                    for ii = 1:24
                        count1 = count1 + 1;
...
ncid=netcdf.create('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\DUST SEVIRI\seviri_data_20150402\output_20150402_new\DG\Seviri_20150401.nc','NETCDF4' );
% ncid=netcdf.create('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\DUST SEVIRI\seviri_data_20150402\output_20150402_new\Seviri_20150402.nc','NOCLOBBER');
dimid(1)=netcdf.defDim(ncid,'lat',1500);
dimid(2)=netcdf.defDim(ncid,'lon',1500);
dimid(3)=netcdf.defDim(ncid,'time',96);
varid1=netcdf.defVar(ncid, 'SEVIRI_DF_2015_04_02' ,'NC_DOUBLE',dimid);
varid2 = netcdf.defVar(ncid, 'lon', 'NC_DOUBLE', dimid(2));
varid3 = netcdf.defVar(ncid, 'lat', 'NC_DOUBLE', dimid(1));
varid4 = netcdf.defVar(ncid, 'time', 'NC_FLOAT', dimid(3));
...
netcdf.create('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\DUST SEVIRI\seviri_data_20150402\output_20150402_new\DG\Seviri_20150401.nc','NETCDF4' );

ncdisp('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\DUST SEVIRI\seviri_data_20150402\output_20150402_new\DG\Seviri_20150401.nc')
dstv=ncread('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\DUST SEVIRI\seviri_data_20150402\output_20150402_new\DG\Seviri_20150401.nc','SEVIRI_DF_2015_04_02');
imagesc(dstv(:,:,3))

ncdisp('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\ECMWF_CAMS\2013\2013-01.nc')

timexx= ncread('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\ECMWF_CAMS\2013\2013-01.nc','time');
%%%%%% end of the script
%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
