...
output_dir <- "F:/Historical_DUST/SEVIRI_DUST_MASK_outputs/TERRA_time_I_Method"
...
# patt_tif <- ".tif"
# filenames_tif <- list.files(output_dir, pattern = patt_tif)
# 
# n_nc_files <- length(filenames)
# n_tif_files <- length(filenames_tif)
# net_filenames <- n_nc_files-n_tif_files
...
###### read the DUST FLAG variable (only)
  ...
  all_rasters <- stack()    # stack ALL HOURS together in an unique raster
    ...
    # save data
    writeRaster(all_rasters, paste0(output_dir,"/", name, "_SEVIRI_TERRA_time_I_Method_MetFr.tif") , options= "INTERLEAVE=BAND", overwrite=T)
    # clear the memory
    gc()
    rm(list=ls())
...
#########################################################################################################
 ...
 output_dir <- "F:/Historical_DUST/SEVIRI_DUST_MASK_outputs/AQUA_time_I_Method"
 ...
 import_nc_seviri <- function(filenames){
   ...
   # save data
   writeRaster(all_rasters, paste0(output_dir,"/", name, "_SEVIRI_AQUA_time_I_Method_MetFr.tif") , options= "INTERLEAVE=BAND", overwrite=T)
   # clear up the memory
   gc()
