...
setwd("F:/Historical_DUST/SEVIRI_DUST_MASK_outputs/HDF5_outputs/I_method_EUMETSAT")

###################################################################################

output_dir <- "F:/Historical_DUST/SEVIRI_DUST_MASK_outputs/HDF5_outputs/I_method_EUMETSAT/daily_sum_I_EUMETSAT"

patt <- ".tif"
filenames <- list.files(pattern = patt)
# filenames <- filenames[3748] # 96 scenes  20150331
n <- length(filenames)
filenames <- filenames[48:n]
...
# inizialise an empty raster to stack ALL HOURS together in an unique raster 
 ...
 import_stack_seviri <- function(filenames){
   ...
   year <- str_sub(filenames, start = 8, end = -18)
   month <- str_sub(filenames, start = 12, end = -16)
   day <- str_sub(filenames, start = 14, end = -14)
   DATE <- paste0(year,"-",month,"-",day)
   n <- dim(stack(filenames))[3]
   name <- str_sub(filenames, start = 8, end = -14)
   ...
   # sum all layer in a raster together
     ...
     plot(sum_rasters)
     writeRaster(sum_rasters, paste0(output_dir,"/", name, "_I_Method_sum.tif") , options= "INTERLEAVE=BAND", overwrite=T)
