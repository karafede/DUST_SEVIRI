...
#############################################################################################################################
import sys ; import datetime as dt ; from rasterstats import zonal_stats ; import numpy as np
...
if cnt_f>5 :                          # Use the threshold count                            
       ...
       lastT=np.genfromtxt('/home/mariners/SEVIRI_DUST/scripts/lastalertTime.csv',delimiter=',',dtype='S')[1]
       d1=dt.datetime.strptime(lastT,'%Y%m%d%H%M') ; d2=dt.datetime.strptime(date,'%Y%m%d%H%M')
       diff=(d2-d1) ; days, seconds = diff.days, diff.seconds
       hours = days * 24 + seconds // 3600 # minutes = (seconds % 3600) // 60 ;  seconds = seconds % 60
       
       if hours >7 :
 
            text='Last Alert Time,'+date ; text=text.split(',') ; print text
            fileNme='/home/mariners/SEVIRI_DUST/scripts/lastalertTime.csv'
            np.savetxt(fileNme,text,fmt='%s',delimiter=',')
            sendmail(alert,email_id)   
       else:
            print "Alert send with in last 8 hour: Do not send now"
