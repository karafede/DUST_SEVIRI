...
output_dir <- "F:/Historical_DUST/SEVIRI_DUST_MASK_outputs/daily_sum"
...
###### read the DUST FLAG variable (only)
  ...
  for (i in 1:dim(var_value)[3]){      # time dimension (always 96 scenes over one day)
    ...
    plot(sum_rasters)
    writeRaster(sum_rasters, paste0(output_dir,"/", name, "_sum.tif") , options= "INTERLEAVE=BAND", overwrite=T)
    # clear memory
    gc()
  }
